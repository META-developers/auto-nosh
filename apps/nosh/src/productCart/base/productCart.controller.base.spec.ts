...
describe("ProductCart", () => {
  ...
  test("GET /productCarts/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/productCarts"}/${nonExistingId}`)
  ...
  test("GET /productCarts/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/productCarts"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /productCarts existing resource", async () => {
