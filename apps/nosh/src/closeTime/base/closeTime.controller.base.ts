/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import * as common from "@nestjs/common";
import * as swagger from "@nestjs/swagger";
import { isRecordNotFoundError } from "../../prisma.util";
import * as errors from "../../errors";
import { Request } from "express";
import { plainToClass } from "class-transformer";
import { ApiNestedQuery } from "../../decorators/api-nested-query.decorator";
import * as nestAccessControl from "nest-access-control";
import * as defaultAuthGuard from "../../auth/defaultAuth.guard";
import { CloseTimeService } from "../closeTime.service";
import { AclValidateRequestInterceptor } from "../../interceptors/aclValidateRequest.interceptor";
import { AclFilterResponseInterceptor } from "../../interceptors/aclFilterResponse.interceptor";
import { CloseTimeCreateInput } from "./CloseTimeCreateInput";
import { CloseTimeWhereInput } from "./CloseTimeWhereInput";
import { CloseTimeWhereUniqueInput } from "./CloseTimeWhereUniqueInput";
import { CloseTimeFindManyArgs } from "./CloseTimeFindManyArgs";
import { CloseTimeUpdateInput } from "./CloseTimeUpdateInput";
import { CloseTime } from "./CloseTime";

@swagger.ApiBearerAuth()
@common.UseGuards(defaultAuthGuard.DefaultAuthGuard, nestAccessControl.ACGuard)
export class CloseTimeControllerBase {
  constructor(
    protected readonly service: CloseTimeService,
    protected readonly rolesBuilder: nestAccessControl.RolesBuilder
  ) {}
  @common.UseInterceptors(AclValidateRequestInterceptor)
  @common.Post()
  @swagger.ApiCreatedResponse({ type: CloseTime })
  @nestAccessControl.UseRoles({
    resource: "CloseTime",
    action: "create",
    possession: "any",
  })
  @swagger.ApiForbiddenResponse({
    type: errors.ForbiddenException,
  })
  async create(@common.Body() data: CloseTimeCreateInput): Promise<CloseTime> {
    return await this.service.create({
      data: {
        ...data,

        timeLapse: {
          connect: data.timeLapse,
        },
      },
      select: {
        hour: true,
        id: true,
        minute: true,

        timeLapse: {
          select: {
            id: true,
          },
        },
      },
    });
  }

  @common.UseInterceptors(AclFilterResponseInterceptor)
  @common.Get()
  @swagger.ApiOkResponse({ type: [CloseTime] })
  @ApiNestedQuery(CloseTimeFindManyArgs)
  @nestAccessControl.UseRoles({
    resource: "CloseTime",
    action: "read",
    possession: "any",
  })
  @swagger.ApiForbiddenResponse({
    type: errors.ForbiddenException,
  })
  async findMany(@common.Req() request: Request): Promise<CloseTime[]> {
    const args = plainToClass(CloseTimeFindManyArgs, request.query);
    return this.service.findMany({
      ...args,
      select: {
        hour: true,
        id: true,
        minute: true,

        timeLapse: {
          select: {
            id: true,
          },
        },
      },
    });
  }

  @common.UseInterceptors(AclFilterResponseInterceptor)
  @common.Get("/:id")
  @swagger.ApiOkResponse({ type: CloseTime })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @nestAccessControl.UseRoles({
    resource: "CloseTime",
    action: "read",
    possession: "own",
  })
  @swagger.ApiForbiddenResponse({
    type: errors.ForbiddenException,
  })
  async findOne(
    @common.Param() params: CloseTimeWhereUniqueInput
  ): Promise<CloseTime | null> {
    const result = await this.service.findOne({
      where: params,
      select: {
        hour: true,
        id: true,
        minute: true,

        timeLapse: {
          select: {
            id: true,
          },
        },
      },
    });
    if (result === null) {
      throw new errors.NotFoundException(
        `No resource was found for ${JSON.stringify(params)}`
      );
    }
    return result;
  }

  @common.UseInterceptors(AclValidateRequestInterceptor)
  @common.Patch("/:id")
  @swagger.ApiOkResponse({ type: CloseTime })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @nestAccessControl.UseRoles({
    resource: "CloseTime",
    action: "update",
    possession: "any",
  })
  @swagger.ApiForbiddenResponse({
    type: errors.ForbiddenException,
  })
  async update(
    @common.Param() params: CloseTimeWhereUniqueInput,
    @common.Body() data: CloseTimeUpdateInput
  ): Promise<CloseTime | null> {
    try {
      return await this.service.update({
        where: params,
        data: {
          ...data,

          timeLapse: {
            connect: data.timeLapse,
          },
        },
        select: {
          hour: true,
          id: true,
          minute: true,

          timeLapse: {
            select: {
              id: true,
            },
          },
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new errors.NotFoundException(
          `No resource was found for ${JSON.stringify(params)}`
        );
      }
      throw error;
    }
  }

  @common.Delete("/:id")
  @swagger.ApiOkResponse({ type: CloseTime })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @nestAccessControl.UseRoles({
    resource: "CloseTime",
    action: "delete",
    possession: "any",
  })
  @swagger.ApiForbiddenResponse({
    type: errors.ForbiddenException,
  })
  async delete(
    @common.Param() params: CloseTimeWhereUniqueInput
  ): Promise<CloseTime | null> {
    try {
      return await this.service.delete({
        where: params,
        select: {
          hour: true,
          id: true,
          minute: true,

          timeLapse: {
            select: {
              id: true,
            },
          },
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new errors.NotFoundException(
          `No resource was found for ${JSON.stringify(params)}`
        );
      }
      throw error;
    }
  }
}
